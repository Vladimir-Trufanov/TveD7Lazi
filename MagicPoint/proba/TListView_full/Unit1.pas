...
begin
  {$IFDEF wince}
  edFindText := ''; //папка, с которой начинаем просмотр ( корневой каталог диска С )
  {$ELSE}
...
begin
   {$IFDEF wince}
   Button2.Caption := 'Root';
   Button4.Caption := 'Usb Disk';
   Button3.Caption := 'Storage Card';
   {$ENDIF}
end;
...
begin {перехожу на С: }//кнопка быстрого перехода на С: 
  {$IFDEF wince}
     edFindText := ''; CurrentDir := '';
  {$ELSE}
     edFindText := 'C:'; CurrentDir := 'C:';
  {$ENDIF}
  OpDir;
end;
...
begin  {перехожу на D }//кнопка быстрого перехода на D:
 {$IFDEF wince}
    edFindText := '\Storage Card'; CurrentDir := '\Storage Card';
 {$ELSE}
    edFindText := 'D:'; CurrentDir := 'D:';
 {$ENDIF}
 OpDir;
end;
...
begin  {перехожу на A }//кнопка быстрого перехода на A: 
  {$IFDEF wince}
    edFindText := '\USB Disk'; CurrentDir := '\USB Disk';
  {$ELSE}
    edFindText := 'A:'; CurrentDir := 'A:';
 {$ENDIF}
 OpDir;
end;

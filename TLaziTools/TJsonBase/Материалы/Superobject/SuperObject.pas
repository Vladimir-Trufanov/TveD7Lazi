...
{$ENDIF}
  ...
  public
    class function Hash(const k: String): Cardinal; virtual;
    constructor Create(const AName: String; Obj: Pointer); virtual;
  ...
  protected
    ...
    function CompareKeyNode(const k: String; h: TSuperAvlEntry): integer; virtual;
    function Insert(h: TSuperAvlEntry): TSuperAvlEntry; virtual;
    function Search(const k: String; st: TSuperAvlSearchTypes = [stEqual]): TSuperAvlEntry; virtual;
  public
    ...
    function Delete(const k: String): ISuperObject;
  ...
  protected
    ...
    procedure PutO(const k: String; const value: ISuperObject);
    function GetO(const k: String): ISuperObject;
    procedure PutS(const k: String; const value: String);
    function GetS(const k: String): String;
    procedure PutI(const k: String; value: SuperInt);
    function GetI(const k: String): SuperInt;
    procedure PutD(const k: String; value: Double);
    function GetD(const k: String): Double;
    procedure PutB(const k: String; value: Boolean);
    function GetB(const k: String): Boolean;
{$IFDEF SUPER_METHOD}
    procedure PutM(const k: String; value: TSuperMethod);
    function GetM(const k: String): TSuperMethod;
{$ENDIF}
    procedure PutN(const k: String; const value: ISuperObject);
    function GetN(const k: String): ISuperObject;
    procedure PutC(const k: String; value: Currency);
    function GetC(const k: String): Currency;
  public
    property O[const k: String]: ISuperObject read GetO write PutO; default;
    property S[const k: String]: String read GetS write PutS;
    property I[const k: String]: SuperInt read GetI write PutI;
    property D[const k: String]: Double read GetD write PutD;
    property B[const k: String]: Boolean read GetB write PutB;
{$IFDEF SUPER_METHOD}
    property M[const k: String]: TSuperMethod read GetM write PutM;
{$ENDIF}
    property N[const k: String]: ISuperObject read GetN write PutN;
    property C[const k: String]: Currency read GetC write PutC;

    function GetValues: ISuperObject;
    function GetNames: ISuperObject;
    function Find(const k: String; var value: ISuperObject): Boolean;
    function Exists(const k: String): Boolean;
  end;
  ...
  public
    ...
    procedure Search(const k: String; st: TSuperAvlSearchTypes = [stEQual]);
    procedure First;
    procedure Last;
...
{$ENDIF}
  ...
  ['{4B86A9E3-E094-4E5A-954A-69048B7B6327}']
    ...
    function Format(const str: String; BeginSep: Char = '%'; EndSep: Char = '%'): String;
...
{$ENDIF}
  ...
  public
    ...
    // parser  ... owned!
    class function ParseString(s: PChar; strict: Boolean; partial: boolean = true; const this: ISuperObject = nil; options: TSuperFindOptions = [];
    ...
    class function ParseFile(const FileName: string; strict: Boolean; partial: boolean = true; const this: ISuperObject = nil; options: TSuperFindOptions = [];
       const put: ISuperObject = nil; dt: TSuperType = stNull): ISuperObject;
    class function ParseEx(tok: TSuperTokenizer; str: PChar; len: integer; strict: Boolean; const this: ISuperObject = nil;
...
const
  ...
  super_hex_chars: PChar = '0123456789abcdef';
...
//  TOK_SR: PChar = '/';
  TOK_NULL: PChar = 'null';
  TOK_CBL: PChar = '{'; // curly bracket left
  TOK_CBR: PChar = '}'; // curly bracket right
  TOK_ARL: PChar = '[';
  TOK_ARR: PChar = ']';
  TOK_ARRAY: PChar = '[]';
  TOK_OBJ: PChar = '{}'; // empty object
  TOK_COM: PChar = ','; // Comma
  TOK_DQT: PChar = '"'; // Double Quote
  TOK_TRUE: PChar = 'true';
  TOK_FALSE: PChar = 'false';
...
begin
  ...
  begin
    p := PChar(Result);
    while p^ <> #0 do
      if p^ <> Char({$if defined(NEED_FORMATSETTINGS)}FormatSettings.{$ifend}DecimalSeparator) then
      inc(p) else
      begin
        p^ := '.';
        Exit;
...
var
  ...
  begin
    if (SOIChar(c) < 256) and (AnsiChar(c) in ['0'..'9']) then
    begin
      Result := True;
      v := v * 10 + Ord(c) - Ord('0');
    end else
...
function SO(const s: String): ISuperObject; overload;
begin
  Result := TSuperObject.ParseString(PChar(s), False);
end;
...
begin
  ...
  for j := 0 to length(Args) - 1 do
    ...
    case TVarRec(Args[j]).VType of
      ...
      vtWideChar: Add(TSuperObject.Create(Char(TVarRec(Args[j]).VWideChar)));
...
{$if declared(vtUnicodeString)}
      vtUnicodeString:
          Add(TSuperObject.Create(String(string(TVarRec(Args[j]).VUnicodeString))));
{$ifend}
    else
      assert(false);
    end;
...
{$if declared(varUString)}
  ...
  {$ELSE}
    varUString:  Result := TSuperObject.Create(String(string(VUString)));
  {$ENDIF}
{$ifend}
  else
...
function UuidFromString(p: PChar; Uuid: PGUID): Boolean;
const
...
type
  ...
  begin
    result := (SOIChar(c) < 256) and (AnsiChar(c) in ['0'..'9', 'a'..'z', 'A'..'Z'])
  end;
var
  pos: Byte;
  state, saved: TState;
...
function StringToUUID(const str: String; var g: TGUID): Boolean;
begin
  Result := UuidFromString(PChar(str), @g);
end;
...
function DoEscape(str: PChar; len: Integer): Integer;
var
...
function DoMinimalEscape(str: PChar; len: Integer): Integer;
var
  pos, start_offset: Integer;
  c: Char;
  begin
...
begin
  ...
  {$ENDIF}
              ...
              begin
                ...
                Append(TOK_DQT, 1);
                if escape then
                  doEscape(PChar(iter.key), Length(iter.key)) else
                  DoMinimalEscape(PChar(iter.key), Length(iter.key));
                if indent then
  ...
  {$ENDIF}
      ...
      stInt:
        begin
          st := IntToStr(FO.c_int);
          Result := Append(PChar(st));
        end;
      stDouble:
        Result := Append(PChar(FloatToJson(FO.c_double)));
      stCurrency:
        begin
          Result := Append(PChar(CurrToJson(FO.c_currency)));
        end;
      stString:
        begin
          Append(TOK_DQT, 1);
          if escape then
            doEscape(PChar(FOString), Length(FOString)) else
            DoMinimalEscape(PChar(FOString), Length(FOString));
          Append(TOK_DQT, 1);
          Result := 0;
        end;
      stArray:
...
class function TSuperObject.ParseString(s: PChar; strict: Boolean; partial: boolean; const this: ISuperObject;
...
begin
  ...
  try
    ...
    size := Stream.Read(PAnsiChar(buffer)^, BUFFER_SIZE);
    SetLength(buffer, size);

    while size > 0 do
    begin
      ParseEx(tok, PChar(String(buffer)), size, strict, this, options, put, dt);
      if tok.err = teContinue then
      begin
        SetLength(buffer, BUFFER_SIZE);
        size := Stream.Read(PChar(String(buffer))^, BUFFER_SIZE);
        SetLength(buffer, size);
      end else
        Break;
    end;
...
const
  ...
  function hexdigit(x: Char): byte; {$IFDEF HAVE_INLINE} inline;{$ENDIF}
  begin
    if x <= '9' then
...
{$ENDIF}
  ...
  function IsEndDelimiter(v: Char): Boolean;
  begin
    if tok.depth > 0 then
      case tok.stack[tok.depth - 1].state of
        tsArrayAdd: Result := AnsiChar(v) in [',', ']', #0];
        tsObjectValueAdd: Result := AnsiChar(v) in [',', '}', #0];
      else
        Result := v = #0;
      end else
        Result := v = #0;
...
{$ENDIF}
    ...
    tsNull:
      begin
        tok.pb.Append(@v, 1);
        if (StrLComp(TOK_NULL, PChar(tok.pb.FBuf), min(tok.st_pos + 1, 4)) = 0) then
        begin
...
{$ENDIF}
    ...
    tsIdentifier:
      begin
        if (this = nil) then
        begin
          if (SOIChar(v) < 256) and IsEndDelimiter(Char(v)) then
          begin
            if not strict then
            begin
              tok.pb.TrimRight;
...
{$ENDIF}
    ...
    tsBoolean:
      begin
        tok.pb.Append(@v, 1);
        if (StrLComp('true', PChar(tok.pb.FBuf), min(tok.st_pos + 1, 4)) = 0) then
        begin
          if (tok.st_pos = 4) then
          if (((SOIChar(v) < 256) and (AnsiChar(v) in path)) or (SOIChar(v) >= 256)) then
            TokRec^.state := tsIdentifier else
        ...
        if (StrLComp('false', PChar(tok.pb.FBuf), min(tok.st_pos + 1, 5)) = 0) then
        begin
...
begin
  ParseString(PChar(path), true, False, self, [foCreatePath, foPutValue], Value);
end;

procedure TSuperObject.PutB(const path: String; Value: Boolean);
begin
  ParseString(PChar(path), true, False, self, [foCreatePath, foPutValue], TSuperObject.Create(Value));
end;
...
begin
  ParseString(PChar(path), true, False, self, [foCreatePath, foPutValue], TSuperObject.CreateCurrency(Value));
end;
...
begin
  Result := ParseString(PChar(path), False, true, self);
  if Result = nil then
...
begin
  if Value = nil then
    ParseString(PChar(path), False, True, self, [foCreatePath, foPutValue], TSuperObject.Create(stNull)) else
    ParseString(PChar(path), False, True, self, [foCreatePath, foPutValue], Value);
end;

function TSuperObject.Delete(const path: String): ISuperObject;
begin
  Result := ParseString(PChar(path), False, true, self, [foDelete]);
end;
...
begin
  Merge(TSuperObject.ParseString(PChar(str), False), true);
end;
...
begin
  Result := ParseString(PChar(path), False, True, Self, [foCreatePath], nil, dataType);
end;

function TSuperObject.Format(const str: String; BeginSep: Char; EndSep: Char): String;
var
  p1, p2: PChar;
begin
  Result := '';
...
var
  obj: ISuperObject;
begin
  obj := ParseString(PChar(path), False, True, Self);
  if obj <> nil then
...
begin
  Result := Validate(TSuperObject.ParseString(PChar(rules), False), TSuperObject.ParseString(PChar(defs), False), callback, sender);
end;
...
var
  ...
  function FindInheritedField(const prop: String; p: ISuperObject): ISuperObject;
  var
    o: ISuperObject;
    e: TSuperAvlEntry;
  begin
...
begin
  Result := Compare(TSuperObject.ParseString(PChar(str), False));
end;
  ...
  case DataType of
    stBoolean:
      case ObjectGetType(obj) of
        ...
        stString:  Result := GetIntCompResult(StrComp(PChar(AsString), PChar(obj.AsString)));
      else
        Result := cpError;
      end;
    stDouble:
...
var
  v: ISuperObject;
begin
  v := ParseString(PChar(path), False, True, Self);
...
begin
  ParseString(PChar(path), False, True, Self, [foCreatePath, foPutValue], TSuperObject.Create(Value));
end;
{$ENDIF}
...
begin
  Result := ParseString(PChar(path), False, True, Self, [foCallMethod], param);
end;
{$ENDIF}
...
begin
  Result := ParseString(PChar(path), False, True, Self, [foCallMethod], TSuperObject.ParseString(PChar(param), False));
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      ReallocMem(FBuf, FSize * SizeOf(Char));
    end;
    // fast move
    move(buf^, FBuf[FBPos], size * SizeOf(Char));
    inc(FBPos, size);
    FBuf[FBPos] := #0;
  end;
end;
...
begin
  while (FBPos > 0) and (SOIChar(FBuf[FBPos-1]) < 256) and (AnsiChar(FBuf[FBPos-1]) in [#32, #13, #10]) do
  begin
    dec(FBPos);
    FBuf[FBPos] := #0;
  end;
...
function TSuperAnsiWriterStream.Append(buf: PChar; Size: Integer): Integer;
var
...
function TSuperUnicodeWriterStream.Append(buf: PChar; Size: Integer): Integer;
begin
...
function TSuperAvlTree.Search(const k: String; st: TSuperAvlSearchTypes): TSuperAvlEntry;
var
...
function TSuperAvlTree.Delete(const k: String): ISuperObject;
var
...
function TSuperAvlTree.CompareKeyNode(const k: String; h: TSuperAvlEntry): integer;
begin
  Result := StrComp(PChar(k), PChar(h.FName));
end;

function TSuperAvlTree.CompareNodeNode(node1, node2: TSuperAvlEntry): integer;
begin
  Result := StrComp(PChar(node1.FName), PChar(node2.FName));
end;
...
procedure TSuperAvlIterator.Search(const k: String; st: TSuperAvlSearchTypes);
var
  h: TSuperAvlEntry;
  d: longint;
  cmp, target_cmp: integer;
...
constructor TSuperAvlEntry.Create(const AName: String; Obj: Pointer);
begin
  FName := AName;
  FPtr := Obj;
  FHash := Hash(FName);
...
class function TSuperAvlEntry.Hash(const k: String): Cardinal;
var
  h: cardinal;
  i: Integer;
begin
...
function TSuperTableString.Find(const k: String; var value: ISuperObject): Boolean;
var
  e: TSuperAvlEntry;
begin
  e := Search(k);
...
function TSuperTableString.Exists(const k: String): Boolean;
begin
  Result := Search(k) <> nil;
end;

function TSuperTableString.GetO(const k: String): ISuperObject;
var
  e: TSuperAvlEntry;
begin
  e := Search(k);
...
procedure TSuperTableString.PutO(const k: String; const value: ISuperObject);
var
...
procedure TSuperTableString.PutS(const k: String; const value: String);
begin
  PutO(k, TSuperObject.Create(Value));
end;

function TSuperTableString.GetS(const k: String): String;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
procedure TSuperTableString.PutI(const k: String; value: SuperInt);
begin
  PutO(k, TSuperObject.Create(Value));
end;

function TSuperTableString.GetI(const k: String): SuperInt;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
procedure TSuperTableString.PutD(const k: String; value: Double);
begin
  PutO(k, TSuperObject.Create(Value));
end;

procedure TSuperTableString.PutC(const k: String; value: Currency);
begin
  PutO(k, TSuperObject.CreateCurrency(Value));
end;

function TSuperTableString.GetC(const k: String): Currency;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
function TSuperTableString.GetD(const k: String): Double;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
procedure TSuperTableString.PutB(const k: String; value: Boolean);
begin
  PutO(k, TSuperObject.Create(Value));
end;

function TSuperTableString.GetB(const k: String): Boolean;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
{$IFDEF SUPER_METHOD}
procedure TSuperTableString.PutM(const k: String; value: TSuperMethod);
begin
  PutO(k, TSuperObject.Create(Value));
end;
{$ENDIF}
...
{$IFDEF SUPER_METHOD}
function TSuperTableString.GetM(const k: String): TSuperMethod;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
procedure TSuperTableString.PutN(const k: String; const value: ISuperObject);
begin
  if value <> nil then
...
function TSuperTableString.GetN(const k: String): ISuperObject;
var
  obj: ISuperObject;
begin
 obj := GetO(k);
